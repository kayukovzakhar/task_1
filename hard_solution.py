...
# "скучных" слов, количества "скучных" слов в текущем и предыдущем предложениях
cnt_sent = cnt_words = curr_words = prev_words = 0
...
for word in text:
    ...
    if word[-1] in "!?":
        ...
        if prev + curr + nxt == "?!?":
            cnt_sent += 1
            cnt_words += prev_words
        # Обновляем счетчик предыдущего предложения, Сбрасываем счетчик "скучных" слов для этого предложения
